...
<script>
    ...
    // opener.document.addBankAccountForm.bankCode.value = v1;
     opener.document.addBankAccountForm.bankName.value = v2;
		window.close();
	}
...
<body>
		...
		action="/erapay/addbankaccountbanksearchresult">
					<div class="row">
						<div class="margin-bottom col-xs-4 form-group">
							<label class="pull-right">Bank Code</label>
						</div>
...
<thead>
			...
			<c:forEach var="bo" items="${sessionScope['oAddBankSearchListBO'].addBankSearchList}" varStatus="varSt">
				<tr>
					<td align="center"><c:out value="${bo.bankCode}" /></td>
					<td><c:out value="${bo.bankName}" /></td>
					<td><a href="#" onclick="doSelect('${bo.bankCode}','${bo.bankName}')">Select
					</a></td>
				</tr>
			</c:forEach>
		</table>
